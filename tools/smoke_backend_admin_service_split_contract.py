...
def main() -> int:
    ...
    assert_true(readiness["splitStatus"] == "admin-schema-field-constraint-contract-v238", "unexpected split status")
    ...
    assert_true("backend/app/api/routes/admin_route_services.py" in readiness["extractedFiles"], "missing route service dependency helper")
    assert_true("backend/app/services/admin_service_legacy_markers.py" in readiness["extractedFiles"], "missing admin service legacy marker file")
    assert_true(readiness["groupCount"] >= 10, "expected route service and legacy marker split groups")
