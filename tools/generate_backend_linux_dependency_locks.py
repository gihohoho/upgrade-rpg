...
LOCKS = {
    "pip-bootstrap": (
        "pip-bootstrap.in",
        ("manylinux_2_17_x86_64", "manylinux2014_x86_64", "any"),
    ),
    "runtime-linux-amd64-py311": (
        "runtime.in",
        ("manylinux_2_17_x86_64", "manylinux2014_x86_64", "any"),
    ),
    "runtime-musllinux-amd64-py311": (
        "runtime.in",
        ("musllinux_1_2_x86_64", "musllinux_1_1_x86_64", "any"),
    ),
    "dev-linux-amd64-py311": (
        "dev.in",
        ("manylinux_2_17_x86_64", "manylinux2014_x86_64", "any"),
    ),
}
...
def check_locks(root: Path) -> dict[str, str]:
    ...
    for output_stem, (input_name, _platforms) in LOCKS.items():
    ...
    musl_runtime = parsed["runtime-musllinux-amd64-py311"]
    ...
    if set(musl_runtime) != set(runtime):
        raise LockError("manylinux and musllinux runtime locks must contain the same packages")
    for name, version in read_input_pins(requirements / "runtime.in").items():
        if runtime[name][0] != version or musl_runtime[name][0] != version:
            raise LockError(f"direct runtime pin differs across platforms: {name}")
    return digests
...
def render_lock(
    ...
    lines = [
        ...
        f"# Target: CPython 3.11 / linux/amd64 / {', '.join(platforms)} / binary wheels only",
...
def write_locks(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="upgrade-rpg-lock-") as temporary:
        ...
        for output_stem, (input_name, platforms) in LOCKS.items():
            payload = resolve_report(
                requirements / input_name,
                report_dir / f"{output_stem}.json",
                platforms,
            )
            output = requirements / f"{output_stem}.lock"
            output.write_text(
                render_lock(input_name, payload, platforms),
                encoding="utf-8",
                newline="\n",
            )
