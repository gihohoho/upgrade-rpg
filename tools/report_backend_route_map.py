...
1. v397은 Vue local fallback과 `pending-unsynced` 사용자 선택 복구를 공통 저장 queue에 연결했습니다. v398은 실제 보유 아이템 snapshot 표시를 연결했으며 v399는 복구본 선기록 뒤 가방↔보관함 묶음 이동·정렬 저장을 연결했습니다. 다음은 legacy 자동 합치기입니다. backend/CAS는 분리합니다.
