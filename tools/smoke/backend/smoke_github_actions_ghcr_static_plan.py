...
def copy_fixture(temp: Path) -> None:
    ...
    mutate_lifecycle(temp, lambda p: (
        ...
        p["githubLiveSettings"].update({"recheckedAtUtc": "2026-07-22T02:30:00Z"}),
        p["closure"].update({
...
def main() -> int:
    ...
    expected_result = {
        "preparation-closed": module.READY_RESULT,
        "authorization-open": module.AUTHORIZATION_OPEN_RESULT,
        "authorization-closed-awaiting-evidence": module.AUTHORIZATION_CLOSED_RESULT,
        "attempt-recorded": module.ATTEMPT_RECORDED_RESULT,
    }[result["publishLifecycleState"]]
    ...
    assert result["publishLifecycleState"] in {
        "preparation-closed",
        "authorization-open",
        "authorization-closed-awaiting-evidence",
        "attempt-recorded",
    }
    assert result["publishGateReady"] is (
        result["publishLifecycleState"] == "authorization-open"
    )
    if result["publishLifecycleState"] == "preparation-closed":
    ...
    lifecycle_mutations = (
        ...
        lambda p: p["observedAttempt"].update({"status": "queued"}),
    )
    ...
    workflow_mutations = (
        ...
        (
            ...
            'build_definition = slsa',
        ),
        (
            'if not build_definition.get("buildType"):',
            'if not slsa.get("buildType"):',
        ),
    ...
    print("OK: v331 GitHub Actions/GHCR verified candidate smoke passed")
    return 0
