...
`docs/generated/VUE_GAME_DOMAIN_DEPENDENCIES.md`에 legacy state·systems·rules의 browser·난수·시각·timer 의존성과 v384 typed domain 분리 경계를 기록합니다. v385~v387 smoke는 마을/HUD와 필드·보스 전투 adapter가 snapshot load/save·전투 timer·난수를 시작하거나 HP·보상·쿨타임을 바꾸지 않는지 확인합니다. 검사는 DB·서버·배포를 변경하지 않습니다.
