...
assert_true("from app.api.routes.admin_route_params import" in route, "admin route params import missing")
assert_true("from app.api.routes.admin_route_error_helpers import build_admin_change_logs_unavailable_payload" in route, "admin route error helper import missing")
...
assert_true("current_user: CurrentUser = ADMIN_CURRENT_USER_DEP" in route, "routes should use current user helper")
assert_true("session: AsyncSession = ADMIN_DB_SESSION_DEP" in route, "routes should use DB session helper")
assert_true("_write_guard: bool = ADMIN_WRITE_GUARD_DEP" in route, "write routes should use write guard helper")
assert_true("build_admin_change_logs_unavailable_payload(" in error_helpers, "change-log unavailable builder missing")
assert_true("build_admin_change_logs_unavailable_payload(" in route, "route should call change-log unavailable builder")
...
assert_true('"splitStatus": "admin-route-params-errors-v210"' in contract, "contract splitStatus should be v210")
assert_true('const VERSION = "v210.backend-admin-route-params-error-helpers"' in entry, "frontend readiness version should be v210")
assert_true('splitStatus: "admin-route-params-errors-v210"' in entry, "frontend splitStatus should be v210")
