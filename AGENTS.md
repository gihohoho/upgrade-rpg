...
- Brevo 계정·발신자·API key, 실제 메일, owner bootstrap, 새 image/static 배포는 실행하지 않았습니다.
...
- 다음 단계는 Brevo sender·전용 API key를 local dotenv에 넣고 backend를 재시작한 뒤 실제 가입→메일→인증→로그인을 검증하는 것입니다.
