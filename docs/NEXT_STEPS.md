...
## 현재 완료: v181 admin create lifecycle guard helper

신규 row 생성 apply 도메인을 더 열지 않고, 이미 열린 생성→삭제→복원 흐름을 안전하게 확인하기 위한 보조 UI를 추가했습니다.
...
5. 변경 이력에서 create 이력을 열고 id 기반 삭제 preview/apply 확인.
6. `create_delete` 이력 보기 버튼으로 삭제 이력을 찾고 복원 preview/apply 확인.
...
3. create/delete/restore 결과 화면의 dependency count 표시 강화.
