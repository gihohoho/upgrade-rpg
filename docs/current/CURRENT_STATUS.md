...
이 문서는 현재 구현과 승인 경계를 설명합니다. 장기 작업 규칙은 루트 `AGENTS.md`, 새 채팅의 바로 다음 행동은 `NEXT_CHAT_HANDOFF.md`가 기준입니다.
...
- 검증된 이메일 정규화를 위해 `email-validator==2.3.0`이 필요합니다. 아직 설치하거나 lock을 바꾸지 않았고, dependency가 없으면 가입 관련 API는 503으로 fail-closed합니다.
- 현재 생성 보고서는 `docs/generated/POSTGRES_ALEMBIC_READINESS.md`입니다. 상세 runbook은 `docs/reference/database/`에 있습니다.
...
- `email-validator` 설치와 Linux lock 갱신
...
1. 기호가 `email-validator==2.3.0` 설치와 Linux lock 갱신을 승인합니다.
2. dependency/lock과 v371 source를 다시 검증해 준비 commit을 만듭니다.
3. 별도 exact-SHA 승인 뒤 isolated PostgreSQL upgrade→downgrade→upgrade를 검증합니다.
4. migration apply, Brevo 설정, 테스트 메일, owner bootstrap, 공개 배포는 각각 다음 승인 단위로 분리합니다.

세부 인증 계약은 `ACCOUNT_EMAIL_VERIFICATION_RECOVERY_AND_DELETION.md`, 기존 캐릭터 저장 계약은 `ACCOUNT_AUTH_AND_CHARACTER_SLOTS.md`, 보안 후속은 `SECURITY_ROTATION_AND_GITHUB_GATES.md`를 봅니다.
