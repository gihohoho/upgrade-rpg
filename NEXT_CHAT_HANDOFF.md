# Upgrade RPG Codex handoff — v387
...
- account master-data 응답의 활성 보스 목록을 Pinia에 보존하고 `bossCombat.ts`가 타입·티어·HP·소환 조건·쿨타임과 typed 보스 스킬 드랍/첫 장비 보정 규칙을 표시 전용 view model로 만듭니다. source 입력은 바꾸지 않습니다.
- 마을의 보스존 버튼은 첫 일반 보스로 이동합니다. 일반/특수 탭, 이전/다음, 6개 묶음 선택은 화면과 `boss.preview.select` action 안내만 바꿉니다.
- 보스 화면에는 마을 전용 접속 캐릭터 바가 렌더링되지 않습니다. 마을→보스, 일반→특수, 보스→마을, desktop/mobile 단일 열·가로 넘침 없음·console 0과 focused smoke가 PASS했고 임시 harness는 제거했습니다.
- 실제 소환·snapshot load/save·자동 저장·전투 timer·HP 감소·난수 드랍·보상·쿨타임 변경, backend·DB·env·secret·legacy·Render는 변경하지 않았고 v387 production 승인/실행은 없습니다.
- v384~v386의 typed domain·마을/HUD·필드 UI와 폐기 예정 `baseUrl` 제거 상태를 유지합니다. 관리자 Apply route/header/write도 계속 없습니다.
...
- 기존 공개 legacy·backend·DB·env·secret·Render는 변경하지 않았고 Vue v382~v387 production 승인/실행은 없습니다.
...
1. 다음 Vue 전환 단계는 빈 칸 유지·첫 빈 칸·수동 정렬 계약을 보존하는 표시 전용 인벤토리·장비 UI를 준비하는 `migrate-vue-game-inventory-equipment-ui-foundation`입니다. 실제 snapshot load/save·아이템 이동/사용·자동 저장은 아직 연결하지 않습니다.
