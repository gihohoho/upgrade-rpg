...
1. 기호가 Brevo Free 계정, sender 소유 확인, transactional privacy 설정과 전용 API key를 준비합니다.
2. `BREVO_API_KEY`와 `BREVO_FROM_EMAIL`을 값 출력 없이 local dotenv에 넣고 backend를 재시작합니다.
3. local에서 실제 테스트 메일 1건과 가입→인증→로그인→복구를 확인합니다.
4. local 검증 뒤에만 untouched Neon의 fresh backup·exact v377 apply를 별도 exact 범위로 진행합니다.
