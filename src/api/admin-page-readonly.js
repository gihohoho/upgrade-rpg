(function () {
  ...
  const VERSION = "v230.backend-admin-openapi-route-contract";
  ...
  const ADMIN_BACKEND_SERVICE_SPLIT_CONTRACT = {
    ...
    smoke: "tools/smoke_backend_admin_openapi_route_contract.py",
  };
...
function getAdminCreateLifecycleApi() {
  ...
  function getAdminBackendServiceSplitContractReadiness() {
    ...
    const ok = contract.status === "contract-frozen-v198"
      && contract.splitStatus === "admin-openapi-route-contract-v230"
  ...
  function renderAdminBackendServiceSplitContractReadiness(contractReadiness) {
    ...
    return `
      <div class="create-lifecycle-card create-lifecycle-card-wide">
        ${renderAdminOperationResultBanner({
          tone: readiness.ok ? "good" : "warn",
          title: readiness.ok ? "백엔드 admin route operation 계약 완료" : "백엔드 admin route operation 확인 필요",
          subtitle: `${readiness.routeFile}의 static ownership, FastAPI runtime 등록 route, endpoint/type metadata를 함께 고정했습니다.`,
          metrics: [
        ...
        <div class="filter-help">검증 smoke: <code>${escapeHtml(readiness.smoke)}</code> + <code>tools/run_smoke_core.sh</code>. v228은 route/schema/path를 그대로 두고 FastAPI runtime 등록 route와 endpoint/type metadata를 static ownership map과 대조합니다.</div>
        <div class="create-blueprint-summary" style="grid-template-columns: minmax(0, 1fr) minmax(0, 1fr);">
