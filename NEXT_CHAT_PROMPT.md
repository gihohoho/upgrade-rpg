...
이번에 첨부하는 최신 ZIP `rpg_v300_postgres_migration_roundtrip_reupgrade_ready.zip`을 반드시 기준으로 작업해주세요.
...
v298 첫 `upgrade head`와 v299 `downgrade base`가 사용자 PC에서 실제 성공했습니다.
...
대상은 `rpg_game_migration_empty_v290`만 허용합니다. exact `upgrade head` 후 23 public tables, revision `v295_initial_schema`, differences=0을 확인하고 첫 v298 upgrade signature와 정확히 비교합니다. source/rehearsal DB는 변경하지 않습니다.

성공 후 다음 단계는 source DB baseline stamp 전략의 read-only preflight입니다. 원본 DB stamp/upgrade/downgrade와 DB 삭제는 아직 실행하지 않습니다.
