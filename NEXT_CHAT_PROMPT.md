...
문서를 읽은 뒤 완료된 v377 source·private environment·local/Neon migration·Brevo E2E·GitHub/GHCR·Render backend 배포, v378 static 배포와 v379~v389 Vue 기반·인증·관리자 경계·typed game domain·마을/HUD·필드·보스·인벤토리/장비·보관함/휴지통 UI를 다시 전면 감사하거나 재실행하지 말고 `migrate-vue-game-skill-enhancement-ui-foundation`을 이어서 진행해주세요. Docker와 로컬 로그인은 기호가 정상 동작을 직접 확인했으므로 반복하지 마세요. 실제 snapshot load/save·스킬 사용·강화·재료 소비·아이템 이동/복구/삭제·자동 저장·전투 timer·난수 orchestration, Apply endpoint·재인증 request·dev key header·DB write와 production 관리자 복구는 각각 exact 범위를 별도 승인받기 전에는 실행하지 마세요. 기존 marker/evidence 삭제·덮어쓰기, 자동 retry, DB reset·seed·restore·stamp·actual downgrade와 남은 공개 gate 우회는 하지 마세요.
