# Upgrade RPG Codex next prompt — v364
...
- 공개 Render Static Site는 아직 v351이므로 v357~v364 변경이 없는 것이 정상입니다.
...
- v364에서 `어둠을 지배하는 고리`의 기본·진·초월·연옥·진 연옥·초월 연옥 6개를 각각 별도 256×256 PNG로 다시 만들었습니다. 굵은 외곽선과 만화식 명암을 쓰고 단계마다 재질·색·룬·마력 효과를 발전시켰습니다.
- 일반 장비 이미지 캐시 식별자는 `?v=364`이며, 변경하지 않은 특수장비 PNG는 `?v=361`을 유지합니다.
- 실제 Chrome에서 10·11·12·18단계 반지가 서로 다른 파일로 표시되고 정사각형 슬롯을 채우며 CSS 등급 테두리도 함께 적용되는 것을 확인했습니다.
- 현재 `src/assets/equipment/`에는 새 반지 6개와 아직 재생성 전인 기존 기본 이미지 14개, 총 20개가 있습니다. fallback은 화면 깨짐을 막기 위한 임시 상태입니다.
...
다음 단계에서는 10·11·12·18·19·20단계의 나머지 4계열을 기본 → 진 → 초월 → 연옥 → 진 연옥 → 초월 연옥 순서로 각각 발전시켜 별도 PNG 24개를 만듭니다. 한 계열의 바로 이전 단계 PNG를 편집 원본으로 사용해 실루엣·각도·구도를 유지합니다. 전체 장비 이미지 작업이 끝날 때까지 Render Static Site deploy를 실행하지 않습니다.
...
v364 focused smoke는 10~20단계 일반 장비 55개의 로컬 이미지, 반지 계열 6단계의 서로 다른 파일, 전체 20개 PNG의 signature·256×256과 정적 배포 포함을 고정합니다. 다음 단계는 `generate-v365-normal-equipment-tiered-icons-tier10-family-remainder`이며 공개 Render Static Site는 계속 v351로 유지합니다. v362 인벤토리, v361 특수장비 이미지, v357 장비 공식과 이전 배포·공급자 baseline도 보존합니다.
