  ...
  <div v-if="boss && town" class="boss-shell" data-zone="boss">
    ...
    <aside class="boss-data-boundary" aria-label="보스 미리보기 데이터 경계">
      ...
      <div>
        ...
        <p>보스 정보는 PostgreSQL master-data, 공격력 계산은 읽어온 서버 snapshot을 사용합니다. 직렬 snapshot 저장은 연결됐지만 현재 HP는 client runtime 전용이며 난수 드랍·보상·쿨타임 변경·자동 재소환은 아직 연결하지 않습니다.</p>
      </div>
    </aside>
  </div>
</template>
