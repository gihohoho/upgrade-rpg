...
- `/game`: 로그인·가입·이메일 인증 안내, 계정별 캐릭터 슬롯 8개 gate, 선택 뒤 마을/HUD·필드·보스·인벤토리/장비 미리보기
...
`/game`은 typed API client와 Pinia account store로 인증 token과 선택 캐릭터를 처리합니다. 마을에서만 접속 캐릭터 바를 표시하고, 필드·보스·인벤토리/장비 화면은 기존 master-data와 기본 typed domain으로 표시 상태만 바꿉니다. 인벤토리 정렬은 빈 칸과 아이템 상대 순서 계약을 확인하는 미리보기이며 실제 snapshot load/save·장착·사용·판매·강화·이동·자동 저장·전투 runtime은 시작하지 않습니다. TypeScript 별칭은 폐기 예정인 `baseUrl` 없이 `paths`의 설정 파일 기준 상대 경로를 사용합니다. `/admin`은 관리자 권한 확인 전에는 렌더링하지 않으며 모든 GET/Preview에 Bearer를 사용합니다. Preview는 `dryRun: true`, Apply와 DB write는 잠금 상태입니다.
